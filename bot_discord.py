...
# Paddle non configuré — mode paiement manuel actif

# Mode paiement manuel (avant activation Paddle)
MANUAL_MODE       = os.getenv('MANUAL_MODE', 'true').lower() == 'true'
CONTACT_DISCORD   = os.getenv('CONTACT_DISCORD', 'omerta_nat')
CONTACT_EMAIL     = os.getenv('CONTACT_EMAIL', 'maresteph06@gmail.com')
PAYPAL_BASIC      = os.getenv('PAYPAL_BASIC',   'https://www.paypal.com/ncp/payment/KCYRPURNDT8VG')
PAYPAL_PRO        = os.getenv('PAYPAL_PRO',     'https://www.paypal.com/ncp/payment/88Z3588KDE8PA')
PAYPAL_PREMIUM    = os.getenv('PAYPAL_PREMIUM', 'https://www.paypal.com/ncp/payment/G594N8KAJANDW')
ORANGE_MONEY_1    = os.getenv('ORANGE_MONEY_1', '55853991')
ORANGE_MONEY_2    = os.getenv('ORANGE_MONEY_2', '44504601')
...
async def upgrade_cmd(interaction: discord.Interaction):
    ...
    def ls_link(tier_key: str) -> str:
        ...
        paypal_links = {
            'basic':   PAYPAL_BASIC,
            'pro':     PAYPAL_PRO,
            'premium': PAYPAL_PREMIUM,
        }
        base = paypal_links.get(tier_key, PAYPAL_BASIC)
...
async def subscribe_cmd(interaction: discord.Interaction, plan: str):
    ...
    base_link = paypal_links.get(plan, PAYPAL_BASIC)
    params      = urllib.parse.urlencode({
    ...
    if MANUAL_MODE:
        ...
        embed.add_field(
            ...
            value=(
                ...
                + f"   [💳 Lien PayPal]({PAYPAL_BASIC if plan == 'basic' else PAYPAL_PRO if plan == 'pro' else PAYPAL_PREMIUM})\n"
...
async def cancel_subscription(interaction: discord.Interaction):
    ...
    portal_url = f'Contactez @{CONTACT_DISCORD} pour annuler votre abonnement'
